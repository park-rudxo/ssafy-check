...
(function (root) {
  ...
  async function provisionPersonalWebhook() {
    ...
    // channel_locked 인 웹훅도 거른다. 이건 "채널이 비공개"라는 뜻이 아니라
    // 웹훅의 "이 채널로 고정" 설정이다. 켜져 있으면 payload 의 channel 을
    // 무시하고 걸린 채널로만 가서, DM으로 보내는 이 확장은 못 쓴다.
    // (이 확장이 만드는 웹훅은 만들 때부터 false 라 아래에서 걸릴 일이 없다.
    //  사람이 통합 메뉴에서 켜둔 경우를 거르기 위한 것이다)
    //
