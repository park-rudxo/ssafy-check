  ...
  function renderEduOwner() {
    ...
    line.textContent = name
      ? `이 브라우저의 주인: ${name}님 — 다른 계정으로 로그인해 체크한 출석은 알리지 않아요. ` +
        `다른 계정이 이 PC를 쓰는데 주인이 하루 동안 안 보이면, 이 PC에 남은 설정을 자동으로 지웁니다 ` +
        `(안 쓰기만 해서는 지워지지 않아요).`
    ...
    ensureOriginPermission().then((perm) => {
      ...
      SsafyMattermost.provisionPersonalWebhook()
        .then((res) => {
          ...
          cleanupArmed = false;
          renderHookCleanup("");
          // 출석 화면을 바로 연다. 연결만 해두고 edu 를 안 연 채 자리를 뜨면
          // 그 사이에 앉은 사람이 주인으로 잡힌다. 지금 키보드 앞에 있는
          // 사람이 주인이 맞으므로, 여기서 바로 확정한다.
